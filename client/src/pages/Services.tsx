...
export function PackageCard({
  ...
  return (
    ...
    >
      ...
      >
      {canInlineEditPackage && (
        <div className="absolute top-3 left-3 z-30 flex flex-col gap-1 rounded-xl border border-white/10 bg-black/60 p-1 text-white/80 opacity-0 transition group-hover:opacity-100">
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-lg border border-white/10 bg-white/10 p-1.5 hover:bg-white/20"
            title="إضافة سطر جديد"
            aria-label={`إضافة سطر جديد لـ ${pkg.name}`}
            onClick={(event) => {
              event.preventDefault();
              event.stopPropagation();
              handleAddFeatureLine();
            }}
          >
          ...
          <div className="grid grid-cols-2 gap-1">
            <button
              type="button"
              className="inline-flex items-center justify-center rounded-lg border border-white/10 bg-white/10 p-1 hover:bg-white/20"
              title="تحريك لأعلى"
              onClick={(event) => {
                event.preventDefault();
                event.stopPropagation();
